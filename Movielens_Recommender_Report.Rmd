...
first_prediction <- data_set %>%
    mutate(predicted_rating = general_biases$mu_hat + b_i + b_u, error = rating - predicted_rating)%>%
...
Before running the above code, it is worthing noticing that:
...
movieIds <- edx_train_test$train %>% 
            group_by(movieId,title) %>% summarise(n=n()) %>% 
            ungroup() %>% select(movieId,title)
...
most_popular_movies <- tibble(group=double(),movieId=double(),reviews=integer())

for(i in group_stats$group){
  temp <- paste(((edx_train_test$train %>% left_join(user_classification, by="userId") %>% group_by(group,movieId, title) %>% summarise(reviews=n()) %>% filter(group==1) %>% arrange(-reviews) %>% ungroup()%>% select(-movieId))[1:5,]  %>% spread(title, reviews) %>% select(-group) %>% colnames(.)),sep = ", ")
  most_popular_movies <-rbind(most_popular_movies,temp)
}
```
