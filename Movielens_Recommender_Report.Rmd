...
The data used in this exercise comes from  the [Movielens 10M Dataset](https://grouplens.org/datasets/movielens/10m/). After using the download code provided in the course, the resulting data frame contains observations with an individual movie rating from a particular user, each with the below attributes:
...
4. **`r colnames(edx)[4]`** : Time stamp indicating when the the user submitted the rating.
...
* $\lambda_1$ :  weighting parameter for *movie biases*.
* $\lambda_2$ :  weighting parameter for *user biases*.


The calculation of these parameters has been written into the function *General_Biases*. Then, these results are used to create the first set of predictions, setting the tuning parameters to zero. The results are evaluated using the provided *RMSE* function.
...
* There will be users with extreme ratings (i.e. who like family movies and dislike horror films). The user bias won't reflect this.
...
This will result in a *user_vector*, profiling each user in the training set. This algorithm, has been written in the *User_Vectoriser* function, present in the attached R file. The below table shows a sample of the user vectors created by this code.
...
<!-- The below chart presents a flow diagramme for this algorithm. -->
...
3. the cut-off value to filter the users to feed into each clustering round (*cut-off*).
...
The optimal solution has the below parameter, which shows also the resulting overall RMSE plus the RMSE for the clustered (RMSE_1) and non-clusters (RMSE_2) groups, along with the proportion of reviews in each method.
...
When compared with the *textbook* method, this procedure has better results:
...
Having settled on a set of optimal clustering parameters, there is still room to attempt improving the RMSE but tuning the $\lambda$ parameters in the movie and user bias equations:
...
Before moving to assess the model against the validation set, there are two points to inspect. The first point was mentioned before and is the RMSE variance when k-fold validation was done. Taking the 5 best results, it is possible to build the below box plot. This graph show that all combinations are well below the RMSE threshold!
...
With all optimised parameters, it is time now to try to predict the ratings for the validation set. This will be done in two parts:

* First, the model will be re-train using all optimal parameters and the whole combined training+test dataset.
...
In terms of meeting the RMSE requirements, this model has met the objective. However, as mentioned and hinted through this report, this model has some limitations:
...
* Second, the issues regarding user variability have not been addressed (e.g. big variance of rating for one user). This is an area of improvement that perhaps will further improve the model if addressed.

Having said that, this model is still relative simple and fast for predictions, which is a big advantage - future work may still try to leverage this feature via further modifications or combination with other methods.

\newpage
# Conclusion
<!-- a conclusion section that gives a brief summary of the report, its limitations and future work -->

In conclusion, this report contains the development of a recommendation system, based on the one presented in the course's textbook. From this starting, the model was improved, segmenting users into groups and re-fitting the model accordingly. Through this re-fitting and further tuning, a RMSE of `r compare_RMSE %>% filter(Model=="Optimal Clusters + Optimised Lambdas") %>% pull(RMSE)` was obtained. This results exceeds the goal of achieving a value lower than `r target_rmse`.

As mentioned in the previous sections, despite its simplicity the resulting model is time-intensive when it comes to fitting. This is mainly due to the clustering process, which potentially requires multiple iterations. In addition, although it provides an improvement to the previous model, there are still many individual cases where the error is greater than acceptable to provide a reliable prediction of whether a user will like or dislike a particular movie.

Of course this model can be improved. Some of the future work that could be done comprises:
